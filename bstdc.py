...
def sqlite_and_sql_server_db(Page_Data, create_db_and_table_name):
    ...
    try:
        ...
        que = "INSERT INTO " + create_db_and_table_name + " (Tender_Summery, Documents_2, Content,  Flag) VALUES (?, ?, ?, 1)"
        conn.executemany(que, Page_Data)
        conn.commit()
        logging.info('SQLite Data Inserted')

        # ----------------------------------------------select sqlite data for insert in sql server -------------------
        sqlite_rows = conn.execute("SELECT Tender_Summery, Documents_2, Content FROM " + create_db_and_table_name + " WHERE Flag = 1").fetchall()
        if len(sqlite_rows) > 0:
            ...
            que1 = "INSERT INTO " + create_db_and_table_name + " (Tender_Summery, Documents_2, Content) VALUES (?, ?, ?)"
...
try:
    ...
    log_files_dir = os.path.expanduser('~') + "\\Documents\\" + "PythonLog\\"
    if os.path.exists(log_files_dir):
        pass
    else:
        os.makedirs(log_files_dir)

    logging.basicConfig(filename=log_files_dir + file_name + ".log", filemode='a', level=logging.DEBUG, format='%(asctime)s %(message)s')
    ...
    for tr_data in tr_all_data:
        ...
        if not 'Corrigendum' in tr_data.text:
            ...
            #
            # sr_no = tr_data.find_element(By.XPATH, value='./td[1]').text
            # print(sr_no)
            ...
            for lnk in links_data:
                if not 'Click To Visit E-tender' in lnk.text:
                    ...
                    links.append(lnk)
            links = ', '.join(links)           # print(links) ### string data     # print(link_list)  ### list data show
            ...
            if not conn.execute(row_exists).fetchone():
                ...
                Row_data = [Tender_Summery, Documents_2, Content]
